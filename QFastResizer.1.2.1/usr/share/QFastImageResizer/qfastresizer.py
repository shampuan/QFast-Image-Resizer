...
class QFastResizer(QWidget):
    ...
    def initUI(self):
        ...
        self.cb_keep_exif.setStyleSheet("margin-left: 5px; font-weight: bold; color: #2c3e50;")
    ...
    def reset_ui(self):
        ...
        self.drop_label.setStyleSheet("border: 2px dashed #1a639b; border-radius: 10px; color: #1a639b; font-weight: bold; background-color: #f9f9f9;")
...
def run_cli(args):
    try:
        ...
        # GUI olmayan ortamlarda hata almamak için dummy app
...
if __name__ == '__main__':
    # Eğer argüman varsa ve ilk argüman dosya yolu DEĞİLSE (mod r/p veya help ise) CLI çalıştır
    if len(sys.argv) > 1 and (sys.argv[1].lower() in ['r', 'p', 'help', '-h', '--help']):
        run_cli(sys.argv[1:])
    else:
        app = QApplication(sys.argv)
        # Sürükle-bırak veya sağ tıkla dosya açma (GUI modu)
